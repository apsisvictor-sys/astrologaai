...
export function RegistrationForm({ onSuccess, onLoginClick }: RegistrationFormProps) {
  const { signUp, signInWithGoogle, signInWithMagicLink, isLoading, error: authError, clearError } = useAuth();
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder="••••••••"
              autoComplete="new-password"
              className={`w-full bg-white/5 border ${errors.confirmPassword ? 'border-red-500' : formData.confirmPassword && formData.password === formData.confirmPassword ? 'border-emerald-500' : 'border-white/10'} rounded-xl px-4 py-3.5 pr-12 text-text-primary placeholder-text-muted focus:outline-none focus:border-primary/50 transition-all`}
            />
            <button
              type="button"

...
export async function calculateNatalChart(birthData: BirthDataInput): Promise<NatalChart> {
  ...
  let birthDataPayload: Record<string, unknown>;
  if (hasCoords) {
    birthDataPayload = {
      year: birthData.year,
...
export async function getCachedChart(_birthData: BirthDataInput): Promise<NatalChart | null> {
  return null;
}

/** @deprecated Redis cache removed — no-op */
export async function invalidateChartCache(_birthData: BirthDataInput): Promise<void> {}

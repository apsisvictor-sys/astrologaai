...
const SIGN_IN_HOUSE_MEANINGS: Record<string, Record<number, { basic: string; basicBg: string; intermediate: string; intermediateBg: string }>> = {
  Aries: {
    1: {
...
const HOUSE_MEANINGS: Record<number, {
  basic: string;
  basicBg: string;
  intermediate: string;
  intermediateBg: string;
...
function getPlanetInSignInterpretation(planet: string, sign: string): { basic: string; basicBg: string; intermediate: string; intermediateBg: string } {
...
export function analyzeChart(chart: NatalChart): ChartAnalysis {
  // Analyze all planets
  const planets: PlanetInterpretation[] = [];
  
  const planetKeys = ['sun', 'moon', 'rising', 'mercury', 'venus', 'mars', 'jupiter', 'saturn', 'uranus', 'neptune', 'pluto', 'northNode', 'southNode', 'chiron'] as const;
  
  for (const key of planetKeys) {
    const position = chart[key] as PlanetPosition | undefined;
  ...
  // Analyze houses
  const houses: HouseInterpretation[] = chart.houses.map((house) => {
  ...
  // Analyze aspects
  const aspects: AspectInterpretation[] = chart.aspects.map((aspect) => {

...
import { SupportedLanguage, DEFAULT_LANGUAGE, detectLanguageFromHeader } from '../middleware/languageDetection';
 ...
 */
export const BULGARIAN_ASTROLOGICAL_TERMS = {
  planets: {
    sun: 'Слънце',
    moon: 'Луна',
    mercury: 'Меркурий',
 ...
 */
export function getLanguageDirectiveWithContext(
  language: SupportedLanguage,
  context: 'chat' | 'forecast' | 'compatibility' | 'alert' | 'tooltip'
): string {
...
export function detectLanguage(
  ...
  if (userPreference && isValidLanguage(userPreference)) {
    return {
      language: userPreference as SupportedLanguage,
 ...
 */
export function normalizeLanguage(code: string | undefined): SupportedLanguage {
  if (!code) return DEFAULT_LANGUAGE;
  
  const normalized = code.toLowerCase().split('-')[0];
  return isValidLanguage(normalized) ? (normalized as SupportedLanguage) : DEFAULT_LANGUAGE;
}
 ...
 */
export function validateBulgarianAstrologicalTerms(content: string): {
  isValid: boolean;
  issues: string[];
} {
  const issues: string[] = [];

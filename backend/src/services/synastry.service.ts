...
const ASPECT_INTERPRETATIONS: Record<string, Record<string, { en: string; bg: string }>> = {
  'sun-sun': {
    conjunction: {
...
function determineAspect(angle: number): { aspect: string; orb: number } | null {
  const aspects = [
...
function getInterpretation(userPlanet: string, partnerPlanet: string, aspect: string): { en: string; bg: string } {
  // Try both orderings
  ...
  const defaults: Record<string, { en: string; bg: string }> = {
    harmonious: {
 ...
 */
export async function calculateSynastryChart(
  ...
  const interAspects: InterPlanetaryAspect[] = [];
 ...
 */
function calculateCompatibilityScore(aspects: InterPlanetaryAspect[]): number {
 ...
 */
function identifyStrengths(aspects: InterPlanetaryAspect[]): RelationshipStrength[] {
  ...
  const sunMoon = harmonious.find(
    a => (a.userPlanet === 'sun' && a.partnerPlanet === 'moon') ||
 ...
 */
function identifyChallenges(aspects: InterPlanetaryAspect[]): RelationshipChallenge[] {
  const challenges: RelationshipChallenge[] = [];
  ...
  const moonSaturn = difficult.find(
    a => (a.userPlanet === 'moon' && a.partnerPlanet === 'saturn') ||
  ...
  const marsSaturn = difficult.find(
    a => (a.userPlanet === 'mars' && a.partnerPlanet === 'saturn') ||
...
function generateSummary(
  ...
  const summaries: Record<string, { en: string; bg: string }> = {
    high: {

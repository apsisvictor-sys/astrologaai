...
interface TimeSensitivityPoint {
  timeOffset: number; // Minutes from original time
...
interface TimeSensitivityResponse {
  profileId: string;
  profileName: string;
  originalTime: {
    time: string;
  ...
  dataPoints: TimeSensitivityPoint[];
  summary: {
    en: string;
    bg: string;
  };
...
function parseBirthTime(time: string | null): { hour: number; minute: number } {
  if (!time) return { hour: 12, minute: 0 }; // Default to noon if unknown
 ...
 */
function formatTimeOffset(baseHour: number, baseMinute: number, offsetMinutes: number): string {
 ...
 */
function calculateHouseChanges(
  originalHouses: HouseCusp[],
  newHouses: HouseCusp[]
): TimeSensitivityPoint['houseChanges'] {
 ...
 */
function calculatePlanetShifts(
  originalChart: NatalChart,
  newChart: NatalChart
): TimeSensitivityPoint['planetShifts'] {
 ...
 */
function calculateRisingSignSensitivity(dataPoints: TimeSensitivityPoint[]): {
  stable: boolean;
  signChanges: number;
 ...
 */
function calculateHouseSensitivity(dataPoints: TimeSensitivityPoint[]): {
  stableHouses: number;
  changingHouses: number[];
  stabilityScore: number;
} {
  ...
  const houseChanges = new Map<number, number>();
...
function generateSummary(
  risingSensitivity: { stable: boolean; signChanges: number; stabilityScore: number },
  houseSensitivity: { stableHouses: number; changingHouses: number[]; stabilityScore: number },
 ...
 */
export async function getTimeSensitivity(req: Request, res: Response) {
  try {
    const { profileId } = req.params;
    const userId = req.user?.id;
    
    // Query parameters
    const timeRange = parseInt(req.query.timeRange as string) || 30; // Default ±30 minutes
    const interval = parseInt(req.query.interval as string) || 5; // Default 5-minute intervals
    ...
    for (const offset of offsets) {
      ...
      const adjustedBirthData: BirthDataInput = {
        ...originalBirthData,
    ...
    const response: TimeSensitivityResponse = {
      profileId,
 ...
 */
export async function getTimeSensitivitySummary(req: Request, res: Response) {
  try {
    const { profileId } = req.params;
    const userId = req.user?.id;
    const lang = (req.query.lang as string) || 'bg';

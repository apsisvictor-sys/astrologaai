...
describe('Error Logger', () => {
  ...
  describe('Basic Logging Functions', () => {
    test('debug should log in development only', () => {
      ...
      errorLogger.debug('Debug message', { test: 'data' });
    ...
    test('info should log informational message', () => {
      errorLogger.info('Info message', { context: 'data' });
    ...
    test('warn should log warning message', () => {
      errorLogger.warn('Warning message', { issue: 'minor' });
    ...
    test('error should log error with code and context', () => {
      errorLogger.error('Error occurred', 'AUTH_INVALID_TOKEN', {
        userId: 'user-123',
        requestId: 'req-456',
        ipAddress: '127.0.0.1',
      });
  ...
  describe('Specialized Logging Functions', () => {
    test('httpError should log HTTP error with context', () => {
      errorLogger.httpError('VALID_REQUIRED_FIELD', 'Validation failed', {
        method: 'POST',
    ...
    test('authError should log authentication error', () => {
      errorLogger.authError('AUTH_INVALID_CREDENTIALS', 'Invalid login attempt', {
        userId: 'user-123',
        ipAddress: '10.0.0.1',
    ...
    test('rateLimitError should log rate limit info', () => {
      errorLogger.rateLimitError('RATE_BURST_LIMIT_EXCEEDED', 'Too many requests', {
        userId: 'user-123',
    ...
    test('databaseError should log database error', () => {
      errorLogger.databaseError('DB_CONNECTION_FAILED', 'Database connection lost', {
    ...
    test('websocketError should log WebSocket error', () => {
      errorLogger.websocketError('WS_CONNECTION_FAILED', 'WebSocket disconnected', {
        socketId: 'ws-abc123',
    ...
    test('aiServiceError should log AI service error', () => {
      errorLogger.aiServiceError('API_AI_SERVICE_UNAVAILABLE', 'AI service down', {
        provider: 'openai',
        model: 'gpt-4',
        tokensUsed: 1500,
  ...
  describe('Helper Functions', () => {
    ...
    describe('Log Level Management', () => {
      test('should get current log level', () => {
        ...
        expect(errorLogger.getLogLevel()).toBe('debug');
        
        mockWinstonLogger.level = 'error';
        expect(errorLogger.getLogLevel()).toBe('error');
      });
  ...
  describe('Error Monitoring', () => {
    ...
    test('initializeMonitoring should log initialization', () => {
      ...
      errorLogger.initializeMonitoring();

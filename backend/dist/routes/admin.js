...
function estimateCostEurCents(inputTokens, outputTokens, model) {
    ...
    const costEur = (inputTokens / 1000000) * rate.input + (outputTokens / 1000000) * rate.output;
...
async function ensureDefaultPrompts() {
    for (const p of DEFAULT_PROMPTS) {
        await prisma_1.prisma.systemPrompt.upsert({
            where: { name: p.name },
...
router.get('/overview', async (req, res) => {
    try {
        ...
        const [totalUsers, tierCounts, newSignups, paidSubs, failedPayments] = await Promise.all([
            ...
            prisma_1.prisma.user.groupBy({ by: ['tier'], _count: { id: true } }),
            prisma_1.prisma.user.count({ where: { createdAt: { gte: start, lte: end } } }),
        ...
        const dailySignups = await prisma_1.prisma.$queryRaw `
...
router.patch('/users/:id/tier', async (req, res) => {
    try {
        ...
        await prisma_1.prisma.user.update({ where: { id: req.params.id }, data: { tier } });
...
router.get('/usage', async (req, res) => {
    ...
    `;
        ...
        const topUsers = heavyUsers.map(u => {
            ...
            const costEur = (tokens / 1000000) * 15;
            return {
                userId: u.user_id,
...
router.get('/revenue', async (req, res) => {
    try {
        ...
        const [activeSubs, cancelledSubs, pastDueSubs] = await Promise.all([
            prisma_1.prisma.subscription.count({ where: { status: 'ACTIVE', tier: { in: ['PRO', 'PREMIUM'] } } }),
            prisma_1.prisma.subscription.count({ where: { status: 'CANCELED', updatedAt: { gte: start, lte: end } } }),
            prisma_1.prisma.subscription.count({ where: { status: 'PAST_DUE' } }),
        ]);
        // MRR breakdown
        const proCount = await prisma_1.prisma.subscription.count({ where: { status: 'ACTIVE', tier: 'PRO' } });
        const premiumCount = await prisma_1.prisma.subscription.count({ where: { status: 'ACTIVE', tier: 'PREMIUM' } });
        const mrrEur = proCount * 10 + premiumCount * 20;
        // New paid subscriptions in date range
        const newPaidSubs = await prisma_1.prisma.subscription.count({
            where: {
                tier: { in: ['PRO', 'PREMIUM'] },
        ...
        if (stripe) {
            try {
                ...
                yearlyCount = stripeSubs.data.filter(s => s.items.data.some(i => i.price.recurring?.interval === 'year')).length;
...
router.get('/prompts', async (_req, res) => {
    try {
        ...
        const prompts = await prisma_1.prisma.systemPrompt.findMany({
            orderBy: { name: 'asc' },
...
router.get('/prompts/:name', async (req, res) => {
    try {
        await ensureDefaultPrompts();
        const prompt = await prisma_1.prisma.systemPrompt.findUnique({
...
router.put('/prompts/:name', async (req, res) => {
    try {
        ...
        const existing = await prisma_1.prisma.systemPrompt.findUnique({ where: { name: req.params.name } });
        if (!existing)
            return res.status(404).json({ success: false, error: { code: 'NOT_FOUND' } });
        const adminEmail = req.user?.email ?? 'admin';
        const newVersion = existing.version + 1;
        // Archive current version
        await prisma_1.prisma.systemPromptHistory.create({
            data: {
        ...
        const updated = await prisma_1.prisma.systemPrompt.update({
            where: { name: req.params.name },
...
router.post('/prompts/:name/restore/:version', async (req, res) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ success: false, error: { code: 'NOT_FOUND' } });
        const historyEntry = await prisma_1.prisma.systemPromptHistory.findFirst({
            where: { promptId: existing.id, version: targetVersion },
        });
        if (!historyEntry)
            return res.status(404).json({ success: false, error: { code: 'VERSION_NOT_FOUND' } });
        const adminEmail = req.user?.email ?? 'admin';
        const newVersion = existing.version + 1;
...
router.put('/config/models', async (req, res) => {
    try {
        ...
        const updates = [];
        if (model_free)
            updates.push({ key: 'model_free', value: model_free });
        if (model_pro)
            updates.push({ key: 'model_pro', value: model_pro });
        if (model_premium)
            updates.push({ key: 'model_premium', value: model_premium });
        if (updates.length === 0) {
            return res.status(400).json({ success: false, error: { code: 'NO_UPDATES' } });
        }
        await Promise.all(updates.map(u => prisma_1.prisma.adminConfig.upsert({
            where: { key: u.key },
...
router.get('/discounts', async (req, res) => {
    try {
        ...
        if (statusFilter === 'active')
            ...
            where.usesCount = { gte: prisma_1.prisma.discountCode.fields.maxUses };
        }
        const codes = await prisma_1.prisma.discountCode.findMany({
            where: {},
            orderBy: { createdAt: 'desc' },
        });
...
router.post('/discounts', async (req, res) => {
    try {
        ...
        const discount = await prisma_1.prisma.discountCode.create({
            data: {
...
router.patch('/discounts/:id', async (req, res) => {
    try {
        ...
        const updated = await prisma_1.prisma.discountCode.update({
            where: { id: req.params.id },
            data: { isActive },
        });
...
router.get('/referrals', async (_req, res) => {
    try {
        const links = await prisma_1.prisma.referralLink.findMany({
            include: {
                conversions: {
...
router.patch('/referrals/:id', async (req, res) => {
    try {
        const { isActive } = req.body;
        const updated = await prisma_1.prisma.referralLink.update({
            where: { id: req.params.id },
            data: { isActive },
        });

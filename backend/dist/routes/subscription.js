...
async function getUserUsage(userId) {
    ...
    const stats = await (0, queryLimit_1.getUserUsageStats)(userId, tier);
    return {
    ...
    try {
        ...
        if (req.headers.authorization) {
            try {
                ...
                if (userId) {
                    userSubscription = await prisma_1.prisma.subscription.findUnique({
                        where: { userId },
                    });
...
router.get('/status', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const usageStats = await (0, queryLimit_1.getUserUsageStats)(userId, effectiveTier);
        // Check current query limit status
        const limitStatus = await (0, queryLimit_1.checkQueryLimit)(userId, effectiveTier);
        // Build response
        const response = {
            tier: effectiveTier,
            ...
            tierConfig: subscription_tiers_1.TIER_CONFIG[effectiveTier],
        };
...
router.post('/portal', auth_1.authMiddleware, async (req, res) => {
    try {
        if (!stripe) {
            return res.status(503).json({
                success: false,
...
router.post('/reactivate', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const subscription = await prisma_1.prisma.subscription.findUnique({
            where: { userId },
...
router.get('/invoices', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user.id;
        const subscription = await prisma_1.prisma.subscription.findUnique({
            where: { userId },
                ...
                if (userId && tier) {
                    ...
                    if (user?.email) {
                        ...
                        const amount = billingPeriod === 'yearly'
                            ? plan.price.yearly
                            : plan.price.monthly;
                        await sendSubscriptionConfirmationEmail(user.email, tier, billingPeriod, amount, 'EUR', user.language || 'bg');
                    }
                }
                break;
            }
...
function getLimitsForTier(tier) {
    const plan = SUBSCRIPTION_PLANS[tier];
    if (!plan)
        return { queries: 10, partners: 0, conversations: 'limited' };
    return {

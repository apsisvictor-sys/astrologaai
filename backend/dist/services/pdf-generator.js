 ...
 */
function generateChartWheelImage(chart, size = 500) {
    ...
    const majorAspects = chart.aspects.filter(a => ['conjunction', 'trine', 'sextile', 'square', 'opposition'].includes(a.aspect));
...
async function generateNatalChartPDF(data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const birthDateFormatted = new Date(data.birthDate).toLocaleDateString(isBulgarian ? 'bg-BG' : 'en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
            ...
            doc.fontSize(8)
                .fillColor(COLORS.textMuted)
                .text(`${t.page} 2`, 50, doc.page.height - 30, { align: 'center' });
            ...
            doc.fontSize(8)
                .fillColor(COLORS.textMuted)
                .text(`${t.page} 3`, 50, doc.page.height - 30, { align: 'center' });
            // Finalize PDF
            doc.end();
        }
        catch (error) {
 ...
 */
function getPDFHeaders(filename) {
    return {

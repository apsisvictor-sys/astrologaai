...
function calculateEmotionalScore(synastry) {
    ...
    for (const aspect of synastry.interAspects) {
        const isEmotional = emotionalPlanets.includes(aspect.userPlanet) ||
    ...
    const moonMoon = synastry.interAspects.find(a => a.userPlanet === 'moon' && a.partnerPlanet === 'moon');
...
function calculateCommunicationScore(synastry) {
    ...
    let totalWeight = 0;
    for (const aspect of synastry.interAspects) {
        const isCommunication = communicationPlanets.includes(aspect.userPlanet) ||
...
function calculatePhysicalScore(synastry) {
    ...
    let totalWeight = 0;
    for (const aspect of synastry.interAspects) {
        const isPhysical = physicalPlanets.includes(aspect.userPlanet) ||
    ...
    const venusMars = synastry.interAspects.find(a => (a.userPlanet === 'venus' && a.partnerPlanet === 'mars') ||
...
function calculateLongTermScore(synastry) {
    ...
    let totalWeight = 0;
    for (const aspect of synastry.interAspects) {
        const isLongTerm = longTermPlanets.includes(aspect.userPlanet) ||
    ...
    const sunSaturn = synastry.interAspects.find(a => (a.userPlanet === 'sun' && a.partnerPlanet === 'saturn') ||
...
Keep the analysis specific, practical, and encouraging.`;
    try {
        const analysis = await (0, llm_1.chatCompletion)([{ role: 'user', content: prompt }], { temperature: 0.7, maxTokens: 200 });
...
Write 3-4 practical pieces of advice that will help them develop their relationship. Each piece of advice should be one sentence.`;
    try {
        const advice = await (0, llm_1.chatCompletion)([{ role: 'user', content: prompt }], { temperature: 0.7, maxTokens: 300 });
        return advice.trim();
    }
    catch (error) {
...
async function generateCompatibilityReport(userBirthData, partnerBirthData, partnerId, partnerName, userId, language = 'bg') {
    ...
    const synastry = await (0, synastry_service_1.calculateSynastryChart)(userBirthData, partnerBirthData, userId, partnerId);
    ...
    const overallScore = Math.round(emotionalScore * 0.3 +
        communicationScore * 0.25 +
        physicalScore * 0.2 +
        longTermScore * 0.25);
    // Get relevant aspects for each category
    const emotionalAspects = synastry.interAspects.filter(a => ['moon', 'venus'].includes(a.userPlanet) || ['moon', 'venus'].includes(a.partnerPlanet));
    const communicationAspects = synastry.interAspects.filter(a => a.userPlanet === 'mercury' || a.partnerPlanet === 'mercury');
    const physicalAspects = synastry.interAspects.filter(a => ['mars', 'venus'].includes(a.userPlanet) && ['mars', 'venus'].includes(a.partnerPlanet));
    const longTermAspects = synastry.interAspects.filter(a => ['saturn', 'jupiter', 'northNode'].includes(a.userPlanet) ||
        ['saturn', 'jupiter', 'northNode'].includes(a.partnerPlanet));
    // Generate category analyses in parallel
    const [emotionalAnalysis, communicationAnalysis, physicalAnalysis, longTermAnalysis,] = await Promise.all([
    ...
    const keyAspects = synastry.interAspects
    ...
    const strengths = synastry.strengths.map(s => language === 'bg' ? `${s.title.bg}: ${s.description.bg}` : `${s.title.en}: ${s.description.en}`);
    const challenges = synastry.challenges.map(c => language === 'bg' ? `${c.title.bg}: ${c.description.bg}` : `${c.title.en}: ${c.description.en}`);
    ...
    // Cache the report
    try {
        await redis_1.redisClient.setEx(cacheKey, REPORT_CACHE_TTL, JSON.stringify(report));
...
//# sourceMappingURL=compatibility-report.service.js.map
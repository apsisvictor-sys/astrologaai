...
module.exports = __toCommonJS(astrology_orchestrator_exports);
...
const HEALTH_CHECK_INTERVAL_MS = parseInt(process.env.ASTROLOGY_HEALTH_CHECK_INTERVAL || "60000", 10);
const MAX_SWITCH_HISTORY = 100;
const UNHEALTHY_THRESHOLD = 3;
const RECOVERY_THRESHOLD = 2;
const HEALTH_CACHE_TTL = parseInt(process.env.ASTROLOGY_HEALTH_CACHE_TTL || "300", 10);
...
async function logAPIFailure(entry) {
  ...
  try {
    ...
    await import_redis.redisClient.lPush(logKey, JSON.stringify(entry));
    await import_redis.redisClient.lTrim(logKey, 0, 999);
  } catch (error) {
...
class AstrologyOrchestrator {
   ...
   */
  initializeProviders() {
    const primaryProvider = (0, import_astrology_api_provider.createAstrologyAPIProvider)();
  ...
  async storeSwitchEvent(event) {
    try {
      ...
      await import_redis.redisClient.lPush(key, JSON.stringify(event));
      await import_redis.redisClient.lTrim(key, 0, MAX_SWITCH_HISTORY - 1);
    } catch (error) {
  ...
  findNextHealthyProvider(fromIndex) {
    for (let i = 0; i < this.providers.length; i++) {
      ...
      if (provider.isAvailable()) {
        ...
        if (circuitBreaker.state === import_astrology_provider.CircuitState.OPEN) {
          continue;
        }
        if (health.status !== import_astrology_provider.AstrologyProviderStatus.UNHEALTHY) {
          return index;
        }
      }
    }
  ...
  trackFailure(providerName, error) {
    ...
    if (failures >= UNHEALTHY_THRESHOLD) {
      ...
      if (provider) {
        ...
        provider.updateHealth(import_astrology_provider.AstrologyProviderStatus.UNHEALTHY, metrics.health.latencyMs, error);
  ...
  trackSuccess(providerName, latencyMs) {
    ...
    if (successes >= RECOVERY_THRESHOLD) {
      const provider = this.providers.find((p) => p.name === providerName);
      if (provider) {
        provider.updateHealth(import_astrology_provider.AstrologyProviderStatus.HEALTHY, latencyMs);
      }
    }
  }
  /**
  ...
  async getCachedHealth() {
    try {
      const cached = await import_redis.redisClient.get(HEALTH_CACHE_KEY);
      if (cached) {
  ...
  async cacheHealth(healthData) {
    try {
      await import_redis.redisClient.setEx(HEALTH_CACHE_KEY, HEALTH_CACHE_TTL, JSON.stringify(healthData));
    } catch (error) {
    ...
    if (cachedHealth) {
      return this.providers.map((provider) => {
        return cachedHealth[provider.name] || {
          status: import_astrology_provider.AstrologyProviderStatus.UNKNOWN,
          latencyMs: 0,
    ...
    const results = await Promise.all(
      this.providers.map(async (provider) => {
        try {
          ...
          return {
            status: import_astrology_provider.AstrologyProviderStatus.UNHEALTHY,
            latencyMs: 0,
            lastCheck: /* @__PURE__ */ new Date(),
            errorCount: 1,
            successCount: 0,
  ...
  async forceRefreshHealth() {
    try {
      await import_redis.redisClient.del(HEALTH_CACHE_KEY);
    } catch (error) {
  ...
  startHealthCheckPolling(intervalMs = HEALTH_CHECK_INTERVAL_MS) {
    ...
    this.healthCheckInterval = setInterval(async () => {
      try {
        ...
        if (activeHealth.status === import_astrology_provider.AstrologyProviderStatus.UNHEALTHY) {
  ...
  getStatus() {
    const healthyCount = this.providers.filter((p) => {
      ...
      return health.status === import_astrology_provider.AstrologyProviderStatus.HEALTHY || health.status === import_astrology_provider.AstrologyProviderStatus.DEGRADED;
    }).length;
    const status = {
  ...
  findBestProvider() {
    const availableProviders = this.providers.map((p, index) => ({
      ...
      ({ provider, metrics, circuitBreaker }) => provider.isAvailable() && metrics.health.status !== import_astrology_provider.AstrologyProviderStatus.UNHEALTHY && circuitBreaker.state !== import_astrology_provider.CircuitState.OPEN
    );
    ...
    availableProviders.sort((a, b) => {
      const healthPriority = {
        [import_astrology_provider.AstrologyProviderStatus.HEALTHY]: 0,
        [import_astrology_provider.AstrologyProviderStatus.DEGRADED]: 1,
        [import_astrology_provider.AstrologyProviderStatus.UNKNOWN]: 2,
        [import_astrology_provider.AstrologyProviderStatus.UNHEALTHY]: 3
      };
  ...
  async getFailureLogs(limit = 100) {
    try {
      const logs = await import_redis.redisClient.lRange("astrology:failure_logs", 0, limit - 1);
...
var astrology_orchestrator_default = AstrologyOrchestrator;

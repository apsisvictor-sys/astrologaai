...
exports.getDailyTransits = getDailyTransits;
 ...
 */
function calculateJulianDay(year, month, day, hour = 12) {
    if (month <= 2) {
        year -= 1;
        month += 12;
    }
 ...
 */
async function getDailyTransits(date) {
    ...
    const jd = calculateJulianDay(date.getFullYear(), date.getMonth() + 1, date.getDate(), date.getHours() + date.getMinutes() / 60);
    ...
    // Cache for 1 hour
    try {
        await redis_1.redisClient.setEx(cacheKey, TRANSIT_CACHE_TTL, JSON.stringify(result));
    }
    catch (error) {

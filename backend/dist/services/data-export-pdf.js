...
async function generateDataExportPDF(data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Generated date
            doc.fontSize(10)
                .text(`${t.generatedOn}: ${new Date(data.exportInfo.exportedAt).toLocaleDateString(isBulgarian ? 'bg-BG' : 'en-GB', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}`, 50, 80, { align: 'center' });
            doc.y = 120;
            // ==================== ACCOUNT INFO ====================
            ...
            const accountInfo = [
                ...
                { label: t.memberSince, value: new Date(data.profile.createdAt).toLocaleDateString(isBulgarian ? 'bg-BG' : 'en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) },
            ];
            let infoY = doc.y + 15;
            ...
            if (data.birthData) {
                ...
                doc.fillAndStroke(COLORS.surface, '#E2E8F0');
                const birthInfo = [
                    { label: t.date, value: new Date(data.birthData.date).toLocaleDateString(isBulgarian ? 'bg-BG' : 'en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) },
            ...
            if (data.birthProfiles.length > 0) {
                ...
                data.birthProfiles.forEach((profile, index) => {
                    ...
                    doc.fillColor(COLORS.textSecondary)
                        .fontSize(10)
                        .font('Helvetica')
                        .text(`${new Date(profile.birthDate).toLocaleDateString()} - ${profile.locationName}`, 70, doc.y + 32);
                    doc.y += 75;
                });
            }
            // Footer
            ...
            if (data.chatHistory.length > 0) {
                data.chatHistory.forEach((session, index) => {
                    ...
                    doc.fillColor(COLORS.textSecondary)
                        .fontSize(10)
                        .font('Helvetica')
                        .text(`${new Date(session.createdAt).toLocaleDateString()} - ${session.messages.length} ${t.messages}`, 70, doc.y + 32);
            ...
            if (data.partners.length > 0) {
                data.partners.forEach((partner) => {
                    ...
                    doc.fillColor(COLORS.textSecondary)
                        .fontSize(10)
                        .font('Helvetica')
                        .text(`${t.relationshipType}: ${partner.relationshipType} | ${new Date(partner.birthDate).toLocaleDateString()}`, 70, doc.y + 32);
                    if (partner.notes) {
                        doc.fillColor(COLORS.textMuted)
                            .fontSize(9)

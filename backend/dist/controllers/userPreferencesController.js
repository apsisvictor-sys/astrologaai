...
async function updatePreferences(req, res, next) {
    try {
        ...
        if (language !== undefined) {
            if (!languageDetection_1.SUPPORTED_LANGUAGES.includes(language)) {
                res.status(400).json({
                    success: false,
                    error: {
                        ...
                        details: [
                            {
                                field: 'language',
                                message: `Language must be one of: ${languageDetection_1.SUPPORTED_LANGUAGES.join(', ')}`,
                            },
                        ],
                    },
                });
        ...
        if (notificationUpdates !== undefined) {
            const currentProfile = await prisma_1.default.profile.findUnique({
...
async function detectLanguage(req, res) {
    ...
    const detectedLanguage = (0, languageDetection_1.detectLanguageFromHeader)(acceptLanguage);
    res.status(200).json({
        success: true,
        data: {
            detectedLanguage,
...
async function unsubscribeFromEmails(req, res, next) {
    try {
        ...
        const { JWT_SECRET } = await Promise.resolve().then(() => __importStar(require('../utils/jwt')));
        let decoded;
        try {

...
function generateAccessToken(userId, email, tier) {
    return jwt.sign({ sub: userId, email, tier }, jwt_1.JWT_SECRET, { expiresIn: jwt_1.JWT_CONFIG.expiresIn });
}
/**
 * Generate refresh token
 */
function generateRefreshToken(userId) {
    return jwt.sign({ sub: userId, type: 'refresh' }, jwt_1.JWT_SECRET, { expiresIn: jwt_1.JWT_CONFIG.refreshExpiresIn });
}
function handleAuthInfraError(error, res) {
    const isPrismaInfraError = error instanceof client_1.Prisma.PrismaClientKnownRequestError ||
...
async function register(req, res, next) {
    try {
        ...
        if (!validationResult.success) {
            res.status(400).json({
                ...
                error: {
                    ...
                    details: (0, validation_1.formatZodErrors)(validationResult.error),
                },
            });
            return;
        }
        const { email, password, fullName, language: bodyLanguage } = validationResult.data;
        ...
        const detectedLanguage = bodyLanguage || (0, languageDetection_1.detectLanguageFromHeader)(acceptLanguage);
        // Hash password
        const saltRounds = 12;
...
async function refresh(req, res, next) {
    try {
        ...
        const decoded = jwt.verify(refreshToken, jwt_1.JWT_SECRET);
...
async function forgotPassword(req, res) {
    try {
        ...
        const { storeResetToken } = await Promise.resolve().then(() => __importStar(require('../utils/redis')));
...
async function resetPassword(req, res, next) {
    try {
        ...
        // Validate reset token
        const { getResetToken, invalidateResetToken, invalidateUserSessions } = await Promise.resolve().then(() => __importStar(require('../utils/redis')));

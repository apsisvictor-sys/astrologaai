...
exports.exportDataSync = exportDataSync;
const prisma_1 = __importDefault(require("../utils/prisma"));
const redis_1 = require("../utils/redis");
const data_export_pdf_1 = require("../services/data-export-pdf");
...
async function requestExport(req, res, next) {
    try {
        ...
        await redis_1.redisClient.setEx(`${EXPORT_KEY_PREFIX}${exportId}`, EXPORT_TTL, JSON.stringify(exportRecord));
        // Also store user's export request in a list (for rate limiting)
        const userExportsKey = `${EXPORT_KEY_PREFIX}user:${userId}`;
        await redis_1.redisClient.lPush(userExportsKey, exportId);
        await redis_1.redisClient.lTrim(userExportsKey, 0, 4); // Keep only last 5 exports
        await redis_1.redisClient.expire(userExportsKey, EXPORT_TTL);
...
async function getExportStatus(req, res, next) {
    try {
        ...
        const recordJson = await redis_1.redisClient.get(`${EXPORT_KEY_PREFIX}${exportId}`);
        if (!recordJson) {
            res.status(404).json({
                success: false,
                error: {
...
async function downloadExport(req, res, next) {
    try {
        ...
        const exportData = await redis_1.redisClient.get(dataKey);
        if (!exportData) {
            res.status(404).json({
                success: false,
                error: {
...
async function listExports(req, res, next) {
    try {
        ...
        const userExportsKey = `${EXPORT_KEY_PREFIX}user:${userId}`;
        const exportIds = await redis_1.redisClient.lRange(userExportsKey, 0, 4);
        // Fetch each export record
        const exports = await Promise.all(exportIds.map(async (id) => {
            const recordJson = await redis_1.redisClient.get(`${EXPORT_KEY_PREFIX}${id}`);
            if (!recordJson)
                return null;
            const record = JSON.parse(recordJson);
            return {
...
async function processExportAsync(exportId, userId, format) {
    ...
    try {
        ...
        if (format === 'json') {
            ...
            // Generate PDF
            const pdfBuffer = await (0, data_export_pdf_1.generateDataExportPDF)(userData);
            exportData = pdfBuffer.toString('base64');
            downloadUrl = `/api/v1/user/export/${exportId}/download`;
        }
        // Store export data
        const dataKey = `${EXPORT_KEY_PREFIX}data:${exportId}`;
        await redis_1.redisClient.setEx(dataKey, EXPORT_TTL, exportData);
        ...
        await updateExportStatus(key, 'failed', undefined, error instanceof Error ? error.message : 'Unknown error');
    }
}
/**
...
async function updateExportStatus(key, status, downloadUrl, error) {
    const recordJson = await redis_1.redisClient.get(key);
    if (!recordJson)
        return;
    const record = JSON.parse(recordJson);
    ...
    if (ttl > 0) {
        await redis_1.redisClient.setEx(key, ttl, JSON.stringify(record));
    }
}
/**
...
async function fetchUserForecasts(userId) {
    ...
    try {
        ...
        const dailyForecast = await redis_1.redisClient.get(dailyKey);
        const weeklyForecast = await redis_1.redisClient.get(weeklyKey);
        if (dailyForecast) {

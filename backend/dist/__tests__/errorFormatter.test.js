...
const errorFormatter_1 = require("../middleware/errorFormatter");
...
describe('Error Formatter Middleware', () => {
    ...
    describe('detectLanguage', () => {
        ...
        test('should handle quality values in Accept-Language header', () => {
            ...
            expect((0, errorFormatter_1.detectLanguage)(req)).toBe('bg'); // bg has higher quality (0.9)
        });
        test('should handle language variants', () => {
            const req = mockRequest({ 'accept-language': 'en-US,en;q=0.9,bg-BG;q=0.8' });
            expect((0, errorFormatter_1.detectLanguage)(req)).toBe('en'); // en-US -> en
        });
    });
    describe('formatErrorResponse', () => {
        test('should format error response in Bulgarian', () => {
            const error = (0, errorFormatter_1.createAppError)('AUTH_INVALID_TOKEN', 'Token is invalid');
            const response = (0, errorFormatter_1.formatErrorResponse)(error, 'bg', 'req-123');
        ...
        test('should format error response in English', () => {
            const error = (0, errorFormatter_1.createAppError)('AUTH_INVALID_TOKEN', 'Token is invalid');
            const response = (0, errorFormatter_1.formatErrorResponse)(error, 'en', 'req-456');
            expect(response.success).toBe(false);
            expect(response.error.code).toBe('AUTH_INVALID_TOKEN');
        ...
        test('should include context variables in error message', () => {
            const error = (0, errorFormatter_1.createAppError)('RATE_QUOTA_EXCEEDED', 'Limit reached', {
                context: { limit: 50 },
            });
            const response = (0, errorFormatter_1.formatErrorResponse)(error, 'bg', 'req-789', { limit: 50 });
    ...
    describe('createAppError', () => {
        test('should create error with code and status', () => {
            const error = (0, errorFormatter_1.createAppError)('VALID_REQUIRED_FIELD', 'Field is required', {
                statusCode: 400,
                context: { field: 'email' },
            });
        ...
        test('should use default status code from error codes', () => {
            const error = (0, errorFormatter_1.createAppError)('AUTH_INVALID_TOKEN', 'Invalid token');
            expect(error.statusCode).toBe(401); // From ErrorHttpStatus mapping
        });
    });
    describe('Helper Functions', () => {
        test('validationError should create validation error', () => {
            const error = (0, errorFormatter_1.validationError)('email', 'Email is required', { value: '' });
        ...
        test('authError should create authentication error', () => {
            const error = (0, errorFormatter_1.authError)('AUTH_INVALID_CREDENTIALS', 'Invalid credentials', {
        ...
        test('notFoundError should create not found error', () => {
            const error = (0, errorFormatter_1.notFoundError)('User', 'user-123', { search: 'by-email' });
        ...
        test('rateLimitError should create rate limit error', () => {
            const error = (0, errorFormatter_1.rateLimitError)('RATE_BURST_LIMIT_EXCEEDED', 60, {
                userId: 'user-123',
                ipAddress: '127.0.0.1',
            });
    ...
    describe('Error Response Structure', () => {
        test('should have consistent error response structure', () => {
            const error = (0, errorFormatter_1.createAppError)('AUTH_INVALID_TOKEN', 'Token invalid');
            const response = (0, errorFormatter_1.formatErrorResponse)(error, 'en', 'test-request');
...
//# sourceMappingURL=errorFormatter.test.js.map